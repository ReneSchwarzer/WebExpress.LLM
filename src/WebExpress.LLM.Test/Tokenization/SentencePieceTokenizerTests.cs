...
public sealed class SentencePieceTokenizerTests
{
    ...
    {
        Assert.Throws<ArgumentException>(() => SentencePieceTokenizer.FromTokenizerJson(null));
    }
    ...
    {
        Assert.Throws<FileNotFoundException>(() =>
            SentencePieceTokenizer.FromTokenizerJson("/nonexistent/tokenizer.json"));
    }
    ...
    public void FromTokenizerJson_WithMissingModelSection_ShouldThrow()
    {
        ...
        try
        {
            ...
            Assert.Throws<InvalidDataException>(() => SentencePieceTokenizer.FromTokenizerJson(tempFile));
        }
        finally
        {
    ...
    public void FromTokenizerJson_WithValidJson_ShouldCreateFunctionalTokenizer()
    {
        ...
        try
        {
            // Minimal tokenizer.json with BPE vocab and merges
            var json = """
            {
                "version": "1.0",
                "model": {
            ...
            var config = TokenizerConfiguration.FromJson("""{ "add_bos_token": false, "add_eos_token": false }""");
    ...
    public void FromTokenizerJson_WithCustomBosEosTokenNames_ShouldResolveFromConfig()
    {
        ...
        try
        {
            // Simulates a Gemma-style tokenizer.json where BOS is "<bos>" and EOS is "<eos>".
            var json = """
            {
                "version": "1.0",
                "model": {
            ...
            // Config specifies Gemma-style token names
            var config = TokenizerConfiguration.FromJson("""
            {
                "bos_token": "<bos>",
                "eos_token": "<eos>",
    ...
    {
        // Simulates a Gemma-style vocabulary where BOS is "<bos>" not "<s>".
        var vocab = new Dictionary<string, int>
        {
            ["<unk>"] = 0,
            ["<bos>"] = 2,
